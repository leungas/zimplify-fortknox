...
CREATE PROCEDURE ZDP_TKOB_SET (ID VARCHAR(1024), DT JSON) 
BEGIN
    INSERT INTO ZIMPLIFY_TASKS(TKOB_ID, TKOB_DATA) 
        VALUES (ID, DT)
        ON DUPLICATE KEY 
            UPDATE TKOB_DATA = DT;
END;
...
CREATE PROCEDURE ZDP_TKOB_DEL (ID VARCHAR(1024)) 
BEGIN   
    DELETE FROM ZIMPLIFY_TASKS WHERE TKOB_ID = ID;
END;
$$
